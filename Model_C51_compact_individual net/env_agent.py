...
class env_agent():
    ...
    def __init__(self):
        ...
        self.bandit_buffer = network.bandit_buffer(3000)
    ...
    def initialize_episode(self):
        ...
        self.init_state_list=[[] for i in range(self.N_station)]
        self.buffer_count=0


    def measure_rnn(self,state,j,tick,station):
        if j > 1 + config.TRAIN_CONFIG['frame_skip']:
            temp_init = self.init_state_list[station][-config.TRAIN_CONFIG['frame_skip']]
        ...
        rnn_value, initial_rnn_state = self.sess.run([self.agent.main_rnn_value[station], self.agent.rnn_out_state],
    ...
    def take_action(self,state,feature,j,tick):
        ...
        for station in range(self.N_station):
            rnn_value, initial_rnn_state = self.measure_rnn(state, j, tick,station)
            self.init_state_list[station].append(initial_rnn_state)
    ...
    def bandit_regret(self):
        ...
        #check the regret
        regret=self.linucb_agent.return_regret(linubc_train[:,4],linubc_train[:,5])
        ...
        if regret<self.regret:
            ...
            if regret/self.regret>1.2: #5% error occured:
                self.regret=1e4 #reset regret threshold
                for i in range(self.N_station):
                    self.linucb_agent.Aa[i]+=0.1*np.identity(self.N_station*4)
                linubc_train = self.bandit_buffer.sample(1000)
                self.linucb_agent.update(linubc_train[:, 4], linubc_train[:, 1], linubc_train[:, 5])
