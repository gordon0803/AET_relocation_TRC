...
class drqn_agent_efficient():
    ...
    def build_main(self):
        ...
        for i in range(self.N_station):
            ...
            localA = tf.layers.dense(streamA, (self.N_station + 1) * self.N, name=myScope_main + 'AW', activation='linear', reuse=None)  # advantage
            Value = tf.layers.dense(streamV, 1, name=myScope_main + 'VW', activation='linear',
                                        reuse=None)  # advantage
            Value2 = tf.layers.dense(streamV2, 1, name=myScope_main + 'VW', activation='linear',
                                         reuse=True)  # advantage

            localA2 = tf.layers.dense(streamA2, (self.N_station + 1) * self.N, name=myScope_main + 'AW', activation='linear',reuse=True)  # advantage
    ...
    def build_target(self):
        ...
        for i in range(self.N_station):
            myScope_main = 'DRQN_Target_'+str(i)
            imageIn = tf.reshape(self.scalarInput, shape=[-1, self.N_station, self.N_station, 5],
                                 name=myScope_main + 'in1')
            input_conv = tf.pad(imageIn, [[0, 0], [2, 2], [2, 2], [0, 0]], "REFLECT",
                                name=myScope_main + 'in2')  # reflect padding!
            ...
            localAdvantage = tf.layers.dense(streamA, (self.N_station+1) * self.N, name=myScope_main + 'AW',activation='linear',reuse=None)  # advantage
            Value = tf.layers.dense(streamV, 1, name=myScope_main + 'VW', activation='linear',
                                        reuse=None)  # advantage
    ...
    def predict(self, rnn,predict_score,e,station,rng,valid,invalid):
       ...
       # print(self.conf)
        ...
        if rng<e: #epsilon greedy
            ...
            Q= self.sess.run(self.mainPredict[station], feed_dict={self.rnn_holder: rnn[0], self.predict_score[station]:[predict_score],self.station_id:[station]})
            action=Q[-1]
