...
class experience_buffer():
    ...
    def sample(self, batch_size, trace_length):
        ll = len(self.buffer)
        v = np.random.randint(ll, size=batch_size)
        sampledTraces = [self.buffer[i] for i in v]
        sampledTraces = np.array(sampledTraces)
        return np.reshape(sampledTraces, [batch_size * trace_length, 13])
...
class bandit_buffer():
        ...
        def recent_sample(self,batch_size):
            ll = len(self.buffer)
            v = np.random.randint(ll-ll//5,ll, size=batch_size)
            sampledTraces = [self.buffer[i] for i in v]
            sampledTraces = np.array(sampledTraces)
